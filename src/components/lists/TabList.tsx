    ...
    <div className="flex flex-col sm:flex-row text-sm md:text-base gap-6 md:gap-10 min-h-[250px]">
      ...
      <div className="font-mono text-xs sm:text-sm relative flex justify-start sm:flex-col overflow-scroll sm:overflow-auto sm:min-w-[180px]">
        {experiences.map(({ company }, i) => (
          <button
            key={getId()}
            className={`h-10 min-w-[120px] sm:w-auto sm:px-5 sm:!text-left capitalize rounded-r-md transition-all duration-200
              hover:bg-violet-500/10 hover:text-accent focus:outline-none focus:bg-violet-500/10 focus:text-accent ${
              i === activeExperience ? 'text-accent bg-violet-500/10 font-semibold' : ''
            }`}
        ...
        {/* Slider */}
        <div className="absolute h-0.5 w-full sm:w-0.5 sm:h-full rounded-full bottom-0 sm:inset-0 left-0 opacity-30" style={{background: 'var(--color-dark-3)'}}></div>
        <div
          style={{...sliderStyle, background: 'linear-gradient(to bottom, var(--color-accent), #22d3ee)'}}
          className="absolute h-0.5 w-[120px] sm:w-0.5 sm:h-10 rounded-full bottom-0 left-0 sm:inset-0 transition-all duration-250 delay-100 in-scroll"
        ></div>
      </div>
